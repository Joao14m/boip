...
def cleanup(signum=None, frame=None):
    ...
    if webhook_id:
        ...
        try:
            requests.delete(f"http://localhost:{BACKEND_PORT}/api/webhooks/{webhook_id}", timeout=5)
        except Exception:
            pass
...
tunnel_log = tempfile.NamedTemporaryFile(mode="w", suffix=".log", delete=False) # creates a temporary file on disk to capture logs
...
for _ in range(30):
    ...
    with open(tunnel_log.name, "r") as f:
        content = f.read()
    match = re.search(r"https://[a-z0-9-]+\.trycloudflare\.com", content) # scan the text looking for an URL
    if match:
...
webhook_token = secrets.token_hex(16) # generates a random 32-char hex string (16 bytes = 32 hex chars)

with open(ENV_FILE, "r") as f: # opens the .env file in read mode. with ensures the file is auto closed after reading
    env_content = f.read()

# Update or add APP_WEBHOOK_URL
if "APP_WEBHOOK_URL=" in env_content:
    env_content = re.sub(r"APP_WEBHOOK_URL=.*", f"APP_WEBHOOK_URL={webhook_url}", env_content)
else:
    env_content += f"\nAPP_WEBHOOK_URL={webhook_url}\n"

# Update or add APP_WEBHOOK_AUTH_TOKEN
if "APP_WEBHOOK_AUTH_TOKEN=" in env_content:
    env_content = re.sub(r"APP_WEBHOOK_AUTH_TOKEN=.*", f"APP_WEBHOOK_AUTH_TOKEN={webhook_token}", env_content)
else:
...
try:
    ...
    old_webhooks = requests.get(f"http://localhost:{BACKEND_PORT}/api/webhooks", timeout=5).json()
    for wh in old_webhooks.get("data", []): # looping through and deleting each one
        ...
        if wh_id: # if id is found, delete
            ...
            requests.delete(f"http://localhost:{BACKEND_PORT}/api/webhooks/{wh_id}", timeout=5)
except Exception as e:
    print(f"  Warning: could not clean old webhooks: {e}")

print("Registering new Asaas webhook...")
try:
    r = requests.post(f"http://localhost:{BACKEND_PORT}/api/webhooks/register", timeout=10)
    data = r.json()
